...
var CategoryTable = function (props) {
  var categoryLookUp = props.categoryLookUp,
    title = props.title,
    selectAction = props.selectAction,
    selectedAttributes = props.selectedAttributes;
  var dispatch = react_redux_1.useDispatch();
  var pieChartData = categoryLookUp.map(function (i) {
    return { angle: i.count };
  });
        ...
        categoryLookUp.map(function (i) {
          return react_1.default.createElement(
            ...
            react_1.default.createElement(
              "th",
              {
                className: selectedAttributes.includes(i.name)
                  ? styles_1.selectedProp
                  : styles_1.unselectedProp,
                onClick: function () {
                  return dispatch(selectAction(i.name));
                },
              },
              i.name
            ),
