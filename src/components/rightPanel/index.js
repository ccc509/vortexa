...
var RightPanel = function () {
  var selectedMaterials = boatMapReducer_1.useTypedSelector(function (state) {
    return state.selectedMaterials;
  });
  var selectedSizes = boatMapReducer_1.useTypedSelector(function (state) {
    return state.selectedSizes;
  });
  var bounds = boatMapReducer_1.useTypedSelector(function (state) {
    return state.bounds;
  });
  var ramps = boatMapReducer_1.useTypedSelector(function (state) {
    return state.ramps;
  });
  ...
  var materialLookups = boatMapReducer_1.useTypedSelector(function (state) {
    return state.materials.map(function (material) {
      return {
        name: material,
        count: helper_functions_1.getNumOfRampsWithMaterial(
          rampsInTheView,
          material
        ),
      };
  ...
  var rampSize = boatMapReducer_1.useTypedSelector(function (state) {
    return state.sizeIntervals.map(function (interval) {
      return {
        name: interval.min + "-" + interval.max,
        count: helper_functions_1.getNumOfRampsInRange(
          rampsInTheView,
          interval
        ),
      };
  ...
  return react_1.default.createElement(
    ...
    react_1.default.createElement(categoryTable_1.CategoryTable, {
      categoryLookUp: materialLookups,
      title: "Material",
      selectAction: boatMapActions_1.selectMaterial,
      selectedAttributes: selectedMaterials,
    }),
    react_1.default.createElement(categoryTable_1.CategoryTable, {
      categoryLookUp: rampSize,
      title: "Size Interval",
      selectAction: boatMapActions_1.selectSize,
      selectedAttributes: selectedSizes.map(function (interval) {
    ...
    react_1.default.createElement(
      "button",
      {
        ...
        onClick: function () {
          return dispatch(boatMapActions_1.clearSelection());
        },
      },
      "Clear Selection"
    )
